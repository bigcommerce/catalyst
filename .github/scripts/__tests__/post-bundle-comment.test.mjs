...
const postBundleComment = require('../post-bundle-comment.js');
...
describe('post-bundle-comment', () => {
  ...
  it('updates existing comment when marker found', async () => {
    ...
    assert.equal(calls.update[0].comment_id, 99, 'Should update the correct comment by id');
  });
  ...
  it('reads report from a custom reportPath', async () => {
    ...
    assert.ok(calls.create[0].body.includes('Custom report content for testing!'));
  });
  ...
  it('passes correct owner, repo, issue_number from context to listComments', async () => {
    ...
    assert.equal(calls.list[0].owner, 'my-org');
    assert.equal(calls.list[0].repo, 'my-repo');
    assert.equal(calls.list[0].issue_number, 123);
  });
  ...
  it('passes correct owner, repo, issue_number from context to createComment', async () => {
    ...
    assert.equal(calls.create[0].owner, 'my-org');
    assert.equal(calls.create[0].repo, 'my-repo');
    assert.equal(calls.create[0].issue_number, 123);
  });
  ...
  it('passes correct owner and repo to updateComment', async () => {
    ...
    assert.equal(calls.update[0].owner, 'org2');
    assert.equal(calls.update[0].repo, 'repo2');
  });
  ...
  it('uses the first comment that contains the marker (not just exact match)', async () => {
    ...
    assert.equal(calls.update[0].comment_id, 2, 'Should update the first matching comment');
  });
