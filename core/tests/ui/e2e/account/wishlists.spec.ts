...
async function logoutInSeparateBrowser(page: Page, browser: Browser) {
  const cookies = await page.context().cookies();

  return test.step('Sign out in a different browser window', async () => {
    const newBrowser = await browser.newContext();

    await newBrowser.addCookies(cookies);
...
test.describe('Account wishlists', () => {
  test(
    ...
    async ({ page, customer }) => {
      ...
      const locator = page.getByTestId('wishlist-list-item').filter({ hasText: wishlistName });
  ...
  test('Wishlists page displays each wishlist correctly', async ({ page, customer }) => {
    ...
    await expect(page.getByTestId('wishlist-list-item')).toHaveCount(2);

    const wishlist1Locator = page
      .getByTestId('wishlist-list-item')
      .filter({ hasText: wishlist1.name });

    const wishlist2Locator = page
      .getByTestId('wishlist-list-item')
      .filter({ hasText: wishlist2.name });
    ...
    await expect(wishlist1Locator.getByTestId('wishlist-actions-menu-button')).toBeVisible();
    ...
    await expect(wishlist2Locator.getByTestId('wishlist-actions-menu-button')).toBeVisible();
  ...
  test.describe('Wishlist actions menu', () => {
    test('Wishlist actions menu displays all actions', async ({ page, customer }) => {
      ...
      const locator = page.getByTestId('wishlist-list-item').filter({ hasText: name });

      await locator.getByTestId('wishlist-actions-menu-button').first().click();
    ...
    test('Share wishlist action is enabled, displays the correct URL for a public wishlist, and copies the URL to clipboard', async ({
      ...
      await locator.getByTestId('wishlist-actions-menu-button').click();
    ...
    test('Rename wishlist action renames a wishlist successfully', async ({ page, customer }) => {
      ...
      await expect(
        page.getByTestId('wishlist-list-item').filter({ hasText: newName }),
      ).toBeVisible();
    });
    ...
    test('Rename wishlist action fails if the user is no longer logged in', async ({
      ...
      await logoutInSeparateBrowser(page, browser);
