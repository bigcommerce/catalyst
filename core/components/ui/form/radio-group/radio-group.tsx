  ...
  return (
    <RadioGroupPrimitive.Root className={className} {...props}>
      {items.map((item) => {
        ...
        return (
          <div className="mb-3 flex items-center w-full gap-2" key={`${id}-${value}`}>
            <RadioGroupPrimitive.Item
              {...itemProps}
              className={cn(
                'hover:border-secondary focus-visible:hover:border-secondary radix-state-checked:hover:border-secondary flex h-[18px] w-[20px] items-center justify-center rounded-full border-2 border-gray-800 bg-white bg-opacity-100 focus-visible:border-primary focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/20 disabled:pointer-events-none disabled:bg-gray-100 radix-state-checked:border-primary radix-state-checked:bg-white radix-state-checked:disabled:border-gray-400 radix-state-checked:disabled:bg-gray-400',
                error &&
