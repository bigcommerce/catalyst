...
check('shipped docs and workflows name only variables the action reads', () => {
  // The check above only asks whether each name appears somewhere. That passed
  // while the setup table named variables the action never reads, because the
  // correct names also appeared further down in the inputs table. This one
  // runs the other way: every configuration name a consumer could copy has to
  // be one the action actually resolves.
  const resolve = action.runs.steps.find((s) => s.name === 'Resolve configuration');
  const canonical = new Set([...resolve.run.matchAll(/:-\$\{([A-Z_]+):-\}\}/g)].map((m) => m[1]));

  // Passed on to later steps under names of their own, so that a job-level
  // variable cannot shadow an explicit input. A consumer never sets these, so
  // naming one in a setup guide sends them to a variable that does nothing.
  const internal = [...resolve.run.matchAll(/echo "([A-Z_]+)=\$/g)]
    .map((m) => m[1])
  ...
  for (const file of facing) {
    ...
    for (const name of new Set(text.match(/\bBIGCOMMERCE_[A-Z_]+\b/g) ?? [])) {
      if (!canonical.has(name)) wrong.push(`${where} names ${name}, which nothing reads`);
    }

    for (const name of internal) {
      if (text.includes(name)) wrong.push(`${where} names ${name}, which is internal`);
    }
  }
