...
// The action resolves this from the `project-uuid` input or the
// BIGCOMMERCE_PREVIEW_PROJECT_UUID variable, then passes it under an internal
// name so a job-level variable cannot shadow an explicit input. Messages name
// the setting the consumer actually owns, not the name it arrives under.
const PROJECT_UUID_SETTING = 'BIGCOMMERCE_PREVIEW_PROJECT_UUID';
...
if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(PROJECT_UUID)) {
  fail(`${PROJECT_UUID_SETTING} is not a UUID: "${PROJECT_UUID}"`);
}
  ...
  if (!project) {
    fail(
      `No hosting project with UUID ${PROJECT_UUID} exists on store ${STORE_HASH}. ` +
        `Check ${PROJECT_UUID_SETTING} against \`catalyst project list\`.`,
    );
  }
