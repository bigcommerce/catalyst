import { transformImageUrl, transformImageUrls, DEFAULT_IMAGE_TRANSFORM_OPTIONS } from '../src/utils/imageTransforms';
...
describe('Image Transform Utils', () => {
  describe('transformImageUrl', () => {
    it('should add lossy parameters to BigCommerce CDN URLs', () => {
      ...
      expect(result).toBe(`${url}?lossy=true&quality=${DEFAULT_IMAGE_TRANSFORM_OPTIONS.lossyQuality}`);
    });
    ...
    it('should not transform URLs that already have lossy parameters', () => {
      const url = 'https://cdn11.bigcommerce.com/s-123abc/products/123/images/456/image.jpg?lossy=true';
      const result = transformImageUrl(url);
    ...
    it('should handle URLs with existing query parameters', () => {
      const url = 'https://cdn11.bigcommerce.com/s-123abc/products/123/images/456/image.jpg?width=100';
      const result = transformImageUrl(url);
