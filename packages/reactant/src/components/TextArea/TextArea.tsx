...
const textAreaVariants = cva(
  'focus:ring-primary-blue/20 h-[64px] w-full border-2 border-gray-200 py-2.5 px-4 hover:border-blue-primary focus:border-blue-primary focus:outline-none focus:ring-4',
  {
    variants: {
      variant: {
        success:
...
export const TextArea = forwardRef<ElementRef<'textarea'>, TextAreaProps>(
  ({ className, required, variant, ...props }, ref) => {
    return (
      <textarea
        className={cs(textAreaVariants({ variant, className }))}
        ref={ref}
        required={required}
        {...props}
      />
