...
test('Docs', async ({ page }) => {
    ...
    await expect(page.frameLocator(storyBookElements.storyBookFrame).locator(docsSlides).getByRole('heading', { name: 'Related Products' })).toBeVisible();
    await expect(page).toHaveScreenshot();
});

test('Multiple slides', async ({ page }) => {
    await page.goto(storyBookElements.docsUrl + '/carousel--multiple-slides');
    await expect(page.frameLocator(storyBookElements.storyBookFrame).getByLabel('Related Products')).toBeVisible();
    await expect(page).toHaveScreenshot();
});
