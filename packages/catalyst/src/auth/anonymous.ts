...
export const anonymousSignIn = async (user: Partial<AnonymousUser> = { cartId: null }) => {
  ...
  /**
   * @todo how do we want to handle dynamic API's such as `await cookies()` and `await headers()`?
   */
  ...
  /**
   * @todo do we need to make sure these cookies are partitioned for makeswift?
   */
