...
The advice adapts to the storefront domain: on a custom domain it suggests the `checkout.<domain>` subdomain to point at BigCommerce, and on an auto-generated deployment hostname it explains that the shared checkout domain is the only option there and that a custom domain is the prerequisite for changing it. The check is advisory and never fails the command that ran it.
