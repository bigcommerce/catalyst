...
1. It asks whether to update that channel's site URL, then prompts only for the hostname (the new deployment is pre-selected). The channel is the one the deploy targets, so it isn't asked for.
